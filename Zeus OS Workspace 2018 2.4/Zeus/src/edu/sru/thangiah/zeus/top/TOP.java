...
public class TOP {
	...
	public TOP(String fileName) {
		...
		double angleBetweenPasses, targetPointDistance, targetEllipseSize; // Information about the target elliptical
																			// area
		...
		if (mainShipments.getTOPHead() == null) {
			...
			targetPointDistance = TOPProblemInfo.getTruckMaxTravelTime()
			...
			targetX = mainDepots.getStartingDepot().getXCoord()
					+ (Math.cos(TOPProblemInfo.getInitialAngle()) * targetPointDistance);
			targetY = mainDepots.getStartingDepot().getYCoord()
					+ (Math.sin(TOPProblemInfo.getInitialAngle()) * targetPointDistance);

			// Determine which shipments can and cannot be added to truck routes due to
			// distance
